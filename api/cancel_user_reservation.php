...
try {
    ...
    $stmt = $conn->prepare("SELECT id FROM reserved_cars WHERE id = ? AND user_id = ? AND status IN ('pending', 'confirmed')");
    ...
    $updateStmt = $conn->prepare("UPDATE reserved_cars SET status = 'cancelled' WHERE id = ?");
