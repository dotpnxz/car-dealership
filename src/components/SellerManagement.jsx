...
const SellerManagement = () => {
  ...
  const handleViewDetails = async (application) => {
    ...
    try {
      ...
      const data = await response.json();
      console.log('Image Data:', data); // Debugging image data

      if (data.success) {
        setModalImages({
          carImages: data.carImages || [],
          idImage: data.idImage || null // Matches the PHP response key
        });
      }
    } catch (err) {
          ...
          <div className="bg-white rounded-lg w-full max-w-6xl max-h-full overflow-y-auto">
            <div className="p-4 sm:p-6">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 lg:gap-8 mt-4">
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid gap-2 text-sm">
                    ...
                    <div className="flex justify-between border-b py-2">
                      ...
                      <span>{selectedCar.seller_contact}</span>
                    </div>
                  </div>
                </div>
                ...
                {/* Car Images */}
                <div>
                  <h4 className="font-semibold text-lg border-b pb-2">Car Photos</h4>
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 mt-2">
                    {modalImages.carImages.map((image, index) => (
                      <div key={index} className="relative">
                        ...
                        />
                        <span className="absolute bottom-1 right-1 bg-black bg-opacity-50 text-white px-2 py-1 rounded text-xs">
                          {image.image_description}
                        </span>
                      </div>
                    ))}
                    {modalImages.carImages.length === 0 && (
                      <div className="bg-gray-100 rounded-lg p-4 text-center text-gray-600 col-span-full">
                  ...
                  {/* Valid ID */}
                  <h4 className="font-semibold text-lg border-b pb-2 mt-4">Valid ID</h4>
                  {modalImages.idImage ? (
                    <div className="relative mt-2">
                      <img
                        src={`http://localhost/car-dealership/uploads/valid_ID/${modalImages.idImage.image_path}`}
                        alt="Valid ID"
                        className="w-full h-48 object-cover rounded cursor-pointer"
                        onClick={() => setFullSizeImage(`http://localhost/car-dealership/uploads/valid_ID/${modalImages.idImage.image_path}`)}
                      />
                    ...
                    <div className="bg-gray-100 rounded-lg p-4 text-center text-gray-600 mt-2">
              ...
              {fullSizeImage && (
                <div
                  className="fixed inset-0 bg-black bg-opacity-80 z-50 flex justify-center items-center cursor-pointer"
